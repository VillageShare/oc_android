...
public class SynchronizeSharedByMeData  extends RemoteDataOperation
                                    implements Runnable{
    
    String TAG =  SynchronizeSharedByMeData.class.getSimpleName();
    HashMap <Integer , String> serverFiles = new HashMap <Integer , String >();
    
    public SynchronizeSharedByMeData(Context context){
        super(context,"getfilessharedbyuser.php");
                ...
                if(array.length() == 0){
                    ...
                    if (((JSONObject)array.get(0)).getString("status").equals("success")){  //1st obj in jsonarray is status:status
                            ...
                            for (int i = 1; i < array.length(); i++){
                                ...
                                String user =  ((JSONObject)array.get(i)).getString("user_location");
                                //Log_OC.d(TAG, "server replied " + id +file[0] +file[1]);
                                //Create a list of downloaded files
                                serverFiles.put(id, user);      
                            }
                            //Update SHARED table on the client
                            mManager.updateSharedByMeFiles(serverFiles);
                            //test
                            //mManager.printTable(ProviderTableMeta.CONTENT_URI_SHARED_BY_ME);
