...
public class PickerActivity extends FragmentActivity {
    ...
    protected void onStart() {
        Log.d("PickerActivity", "onStart()");
        super.onStart();
        if (FRIEND_PICKER.equals(getIntent().getData())) {
            try {
                friendPickerFragment.loadData(false);
                FragmentManager fm = getSupportFragmentManager();
                fm.beginTransaction().show(friendPickerFragment).commit();
                fm.executePendingTransactions();
            } catch (Exception ex) {
                Log.e("PickerAcitivity", "Exception!: " + ex.getMessage());
                onError(ex);
            }
        } else {
            Log.e("PickerAcitivity", "FRIEND_PICKER != getIntent().getData()");
        }
    }
