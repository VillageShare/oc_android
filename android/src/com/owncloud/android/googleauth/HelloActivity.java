...
package com.owncloud.android.googleauth;
...
public class HelloActivity extends Activity {
    ...
    private void handleAuthorizeResult(int resultCode, Intent data) {
        if (data == null) {
            Toast.makeText(getApplicationContext(), "Unknown error authenticating Youtube account, click button again.", Toast.LENGTH_SHORT).show();
            return;
        }
        if (resultCode == RESULT_OK) {
        ...
        if (resultCode == RESULT_CANCELED) {
            ...
            Toast.makeText(getApplicationContext(), "User rejected authorization.", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    public void greetTheUser(View view) {
        ...
        if (statusCode == ConnectionResult.SUCCESS) {
            ...
            Toast.makeText(this, "Unrecoverable Play Services Error", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void getUsername() {
        if (mEmail == null) {
            Log.d(TAG,"mEmail is null");
            pickUserAccount();
            Log.d(TAG, "user account picked");
        } else {
            if (isDeviceOnline()) {
                Log.d(TAG,"Device online");
                getTask(HelloActivity.this, mEmail, SCOPE).execute();
                Log.d(TAG, "getTask on 145 succeeded");
            } else {
